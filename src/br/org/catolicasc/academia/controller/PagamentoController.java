...
import br.org.catolicasc.academia.service.PagamentoService;
...
public class PagamentoController {
	...
	public String savePagamento(@Valid Pagamento pagamentos, BindingResult result, ModelMap model) {
		...
		if (!service.isPagamentoidUnique(pagamentos.getId(), pagamentos.getId())) {
			FieldError idError = new FieldError("pagamento", "id",
					messageSource.getMessage("non.unique.id", new Long[] { pagamentos.getId() }, Locale.getDefault()));
			result.addError(idError);
			return "registration";
		}

		service.save(pagamentos);
	...
	public String editPagamento(@PathVariable Long id, ModelMap model) {
	...
	public String updatePagamento(@Valid Pagamento Pagamento, BindingResult result, ModelMap model,
		...
		if (!service.isPagamentoidUnique(Pagamento.getId(), Pagamento.getId())) {
			FieldError idError = new FieldError("Pagamento", "id",
					messageSource.getMessage("non.unique.id", new Long[] { Pagamento.getId() }, Locale.getDefault()));
			result.addError(idError);
			return "registration";
		}
	...
	public String deletePagamento(@PathVariable Long id) {
